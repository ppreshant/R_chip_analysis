...
{  # finding classes of each column to make it easier for reading the full table
  ...
  deflag <- 1  # get rid of bad flags? (-50 for F, -100 for J)
  ...
  x[c(1:3,6:8,10:13,15:length(x))] <- y[c(5:37,39:length(x))] <- 'NULL'  # we need rows 4 name, 5 ID,9 F median, 14 B median) 
  x[4] <- y[4] <- 'character'
  ...
  a <- cbind(a, 'F/B n' = a[[2]]/(a[[3]]+1)) # normalize foreground to background
